...
import { z } from 'zod';
import { ProjectCreateSchema } from '../../../types/project-management';
import { createProject, listProjects } from '../../../services/projects';

// Enhanced query parameters schema
const ProjectQuerySchema = z.object({
    clientId: z.string().uuid().optional(),
    status: z.enum(['planning', 'active', 'review', 'completed', 'on_hold', 'archived']).optional(),
    priority: z.enum(['low', 'medium', 'high', 'urgent']).optional(),
    page: z.coerce.number().min(1).default(1),
    limit: z.coerce.number().min(1).max(100).default(10),
    search: z.string().optional(),
    sort: z.enum(['name', 'created_at', 'updated_at', 'status', 'priority']).default('created_at'),
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Add security headers and return created project
        const response = createSuccessResponse(result.data, {
            created: true,
        });
